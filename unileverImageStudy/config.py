...
class Config:
    ...
    MAX_CONTENT_LENGTH_FORMS = 100 * 1024 * 1024  # 100MB for form data
    
    # Storage Configuration
    USE_LOCAL_STORAGE = os.environ.get('USE_LOCAL_STORAGE', 'false').lower() in ('true', '1', 'yes', 'on')
    LOCAL_UPLOAD_FOLDER = os.environ.get('LOCAL_UPLOAD_FOLDER') or 'local_uploads'
    ...
    COMPANY_WEBSITE = os.environ.get('COMPANY_WEBSITE', 'https://test.com')
    COMPANY_EMAIL = os.environ.get('COMPANY_EMAIL', 'contact@test.com')
    
    # Feature Descriptions
    FEATURES = {
        'algorithm': {
            'title': 'Advanced RDE Algorithm',
            'description': 'Sophisticated Rule Developing Experimentation with balanced matrix generation for optimal research outcomes.',
            'icon': '🧠'
        },
        'analytics': {
    ...
    # Social Media Links
    SOCIAL_LINKS = {
        'twitter': os.environ.get('SOCIAL_TWITTER', 'https://twitter.com/'),
        'linkedin': os.environ.get('SOCIAL_LINKEDIN', 'https://linkedin.com/company/'),
        'github': os.environ.get('SOCIAL_GITHUB', 'https://github.com/'),
        'youtube': os.environ.get('SOCIAL_YOUTUBE', 'https://youtube.com/')
    }
    ...
    CONTACT_INFO = {
        ...
        'support_email': os.environ.get('SUPPORT_EMAIL', 'support@test.com'),
        'sales_email': os.environ.get('SALES_EMAIL', 'sales@test.com')
    }
